// v43: PWA app shell + default layer 0 + current layer display
...
function setMode(m){
  ...
  if(m !== 'select' && m !== 'pan') selectedIndex = -1;
  updateProperties();
  updateDrawHint();
  draw();
}
...
[snapEnd, snapIntersect, snapCircle, snapPoint, snapNear, snapText].filter(Boolean).forEach(cb => cb.addEventListener('change', ()=>{ syncSnapDock(); draw(); }));
initSnapDock();
...
function snapDockSummary(){
  const pairs = [
    ['snapEnd','端点'],['snapPoint','点'],['snapIntersect','交点'],
    ['snapNear','近接'],['snapText','文字'],['snapCircle','円中心']
  ];
...
canvas.addEventListener('pointerdown', e=>{
  e.preventDefault();
  // 選択済みオブジェクトの四角グリップ。
  // 短く動かせばグリップ編集、長押しなら頂点削除メニュー。
  const grip = (mode==='select' && selectedIndex>=0) ? hitSelectedGripScreen(e.clientX, e.clientY) : null;
  if(grip){
    clearTimeout(longPressTimer);
    pendingGripPress = {
      pointerId:e.pointerId, grip, startX:e.clientX, startY:e.clientY,
      event:{pointerId:e.pointerId, clientX:e.clientX, clientY:e.clientY}
    };
    suppressTap = true;
    canvas.setPointerCapture?.(e.pointerId);
    longPressTimer=setTimeout(()=>{
      if(!pendingGripPress || pendingGripPress.pointerId!==e.pointerId) return;
      showGripMenu(e.clientX, e.clientY, grip);
...
canvas.addEventListener('pointermove', e=>{
  ...
  if(ps.length >= 2){
    ...
    if(lastCentroid && lastPinchDist){
      // Googleマップ風：現在の2本指中点にある図面座標を固定してズーム。
      // これでピンチ中に図面が指から逃げるズレを抑える。
...
canvas.addEventListener('pointerup', e=>{ if(pendingGripPress && pendingGripPress.pointerId===e.pointerId){ clearTimeout(longPressTimer); pendingGripPress=null; suppressTap=true; return; } if(gripDrag && gripDrag.pointerId===e.pointerId){ e.preventDefault(); endGripDrag(e,false); return; } finishPointer(e); }, {passive:false});
canvas.addEventListener('pointercancel', e=>{ if(pendingGripPress && pendingGripPress.pointerId===e.pointerId){ clearTimeout(longPressTimer); pendingGripPress=null; suppressTap=false; return; } if(gripDrag && gripDrag.pointerId===e.pointerId){ endGripDrag(e,true); return; } finishPointer(e, true); }, {passive:false});
canvas.addEventListener('pointerleave', e=>{ if(gripDrag && gripDrag.pointerId===e.pointerId){ return; } if(activePointers.has(e.pointerId)) finishPointer(e, true); }, {passive:false});
...
function handleTapAt(clientX, clientY){
  ...
  const p=nearestSnap(raw, 14/view.scale) || {...raw, snapType:'なし'};
...
function handleDrawingTap(p){
  ...
  if(mode==='line'){
    if(!drawState) { drawState={type:'LINE', pts:[p]}; updateDrawHint(`線分 1点目\n${fmtPtLines(p)}\nスナップ = ${snapLabel(p)}\n2点目をタップ`); draw(); return true; }
  ...
  if(mode==='poly'){
    if(!drawState) drawState={type:'LWPOLYLINE', pts:[]};
    drawState.pts.push(p);
    updateDrawHint(`ポリライン点数: ${drawState.pts.length}\nレイヤ：${currentDrawLayer}\n追加点 ${fmtPtInline(p)}\n続けてタップ、または「確定」`);
    draw();
    return true;
  }
  if(mode==='circle'){
    if(!drawState) { drawState={type:'CIRCLE', center:p}; updateDrawHint(`円 中心点\n${fmtPtLines(p)}\n半径点をタップ`); draw(); return true; }
...
function cancelDrawing(){
  drawState=null; measureStart=null; measureEnd=null; moveState=null; copyState=null; hideObjectMenu();
...
function drawDrawingPreview(){
  ...
  if(drawState.type==='LINE' && drawState.pts.length){ drawCross(drawState.pts[0]); }
  ...
  if(drawState.type==='CIRCLE' && drawState.center){ drawCross(drawState.center); }
  ctx.restore();
}
function saveDXF(){
...
function deleteSelectedEntity(){
  ...
  const e = entities[selectedIndex];
  if(!ensureEditableEntity(e, 'DELETE')) return;
...
function handleMoveTap(raw, snappedPoint){
  // MOVEコマンド：
  // 1) 未選択ならタップ位置のオブジェクトを選択
  // 2) 選択済みなら基準点を指定
  // 3) 次のタップで移動先を指定し、選択オブジェクトを平行移動
  if(selectedIndex < 0 || !entities[selectedIndex]){
  ...
  if(!ensureEditableEntity(entities[selectedIndex], 'MOVE')) return;
  const p = snappedPoint || raw;
  if(!moveState || moveState.index !== selectedIndex){
    moveState = {index:selectedIndex, base:{x:p.x, y:p.y, snapType:p.snapType || 'なし'}};
    command(`MOVE\n基準点 ${fmtPtInline(p)} SNAP=${snapLabel(p)}\n移動先を指定してください。`);
    draw();
    return;
  }
  ...
  const dx = p.x - base.x;
  const dy = p.y - base.y;
  translateEntity(entities[selectedIndex], dx, dy);
  moveState = null;
  refreshGeometry();
  commitHistory();
  updateProperties();
...
function handleSelectTap(raw){
  ...
  if(hit){
    const e=entities[selectedIndex];
    command(`SELECT：${e.type} を選択しました。四角グリップをつかむと端点・頂点・中心だけ移動できます。`);
  } else {
...
function drawSelectedEntity(){
  if(selectedIndex < 0 || !entities[selectedIndex]) return;
  const e=entities[selectedIndex];
  ctx.save();
...
function drawEntityGrips(e){
  ...
  for(const g of grips){
    drawGripSquare(g.point);
  }
  ctx.restore();
}

function drawGripSquare(p){
...
function hitSelectedGripScreen(clientX, clientY){
  if(selectedIndex < 0 || !entities[selectedIndex]) return null;
  const r=canvas.getBoundingClientRect();
...
function startGripDrag(ev, grip){
  ...
  command(`GRIP編集\n${e.type} の ${grip.label} を移動中。移動先で指を離してください。`);
}
...
canvas.addEventListener('pointerdown',e=>{
  ...
  if(mode==='select' && selectedIndex>=0 && !hitSelectedGripScreen(e.clientX, e.clientY)){
    clearTimeout(longPressTimer);
