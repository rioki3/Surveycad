// v62: iPad横画面canvasアスペクト比同期修正
// v62: OFFSET selection flow fixed
...
const currentColorSelect = document.getElementById('currentColorSelect');
const buttons = { pan: modePan, coord: modeCoord, dist: modeDist, select: modeSelect, line: modeLine, poly: modePoly, circle: modeCircle, text: modeText, offset: modeOffset, trim: (typeof modeTrim !== 'undefined' ? modeTrim : null) };
let currentDrawLayer = '0';
let currentDrawColor = localStorage.getItem('surveycad.currentColor') || 'BYLAYER';
...
let trimState = null; // v80.1: TRIM boundary selection/cut phase // v62: OFFSET command state（対象選択→距離入力→方向指定→再クリック確定）
let offsetPromptPending = false; // v62: highlight target before distance prompt
...
// v80.1: AutoCAD基本色（ACI）とBYLAYER対応
const ACI_COLORS = [
  {value:'BYLAYER', label:'BYLAYER', css:null, aci:256},
  {value:'1', label:'赤', css:'#ff3333', aci:1},
  {value:'2', label:'黄', css:'#ffd21f', aci:2},
  {value:'3', label:'緑', css:'#22c55e', aci:3},
  {value:'4', label:'シアン', css:'#00d5ff', aci:4},
  {value:'5', label:'青', css:'#3b82f6', aci:5},
  {value:'6', label:'マゼンタ', css:'#ff4fd8', aci:6},
  {value:'7', label:'白', css:'#e5e7eb', aci:7},
  {value:'8', label:'濃グレー', css:'#6b7280', aci:8},
  {value:'9', label:'薄グレー', css:'#cbd5e1', aci:9},
];
function aciColorDef(v){
  const s = String(v ?? 'BYLAYER').toUpperCase();
  return ACI_COLORS.find(c => String(c.value).toUpperCase() === s || String(c.aci) === s) || ACI_COLORS[0];
}
function colorLabel(v){ return aciColorDef(v).label; }
function normalizeEntityColor(v){
  if(v == null || v === '' || String(v).toUpperCase()==='BYLAYER' || Number(v)===256) return 'BYLAYER';
  const d = aciColorDef(v);
  return d.value === 'BYLAYER' ? 'BYLAYER' : String(d.aci);
}
function entityCssColor(e){
  const c = normalizeEntityColor(e && e.color);
  const d = aciColorDef(c);
  return d.css || '#d1d5db';
}
function applyEntityColor(e){
  const col = entityCssColor(e);
  ctx.strokeStyle = col;
  ctx.fillStyle = col;
}
function makeColorSelect(value, onChange){
  const sel = document.createElement('select');
  sel.className = 'colorSelect';
  for(const c of ACI_COLORS){
    const opt = document.createElement('option');
    opt.value = c.value;
    opt.textContent = c.label;
    opt.selected = normalizeEntityColor(value) === c.value || String(value) === String(c.aci);
    sel.appendChild(opt);
  }
  sel.value = normalizeEntityColor(value);
  sel.onchange = () => onChange(sel.value);
  return sel;
}
function updateCurrentColorUI(){
  if(!currentColorSelect) return;
  currentColorSelect.innerHTML = '';
  for(const c of ACI_COLORS){
    const opt = document.createElement('option');
    opt.value = c.value;
    opt.textContent = c.label;
    opt.selected = normalizeEntityColor(currentDrawColor) === c.value;
    currentColorSelect.appendChild(opt);
  }
  currentColorSelect.value = normalizeEntityColor(currentDrawColor);
  currentColorSelect.title = `現在色: ${colorLabel(currentDrawColor)}`;
}
function setCurrentDrawColor(v, silent=false){
  currentDrawColor = normalizeEntityColor(v);
  localStorage.setItem('surveycad.currentColor', currentDrawColor);
  updateCurrentColorUI();
  if(!silent) command(`現在色：${colorLabel(currentDrawColor)}`);
}
function applyCurrentColorToEntity(e){
  if(!e) return e;
  const c = normalizeEntityColor(currentDrawColor);
  if(c === 'BYLAYER') delete e.color;
  else e.color = Number(c);
  return e;
}
function addColorDXF(add,e){
  const c = normalizeEntityColor(e && e.color);
  if(c !== 'BYLAYER') add(62, Number(c));
}
function displayEntityType(t){
  return ({LINE:'線分', LWPOLYLINE:'ポリライン', CIRCLE:'円', ARC:'円弧', TEXT:'文字', POINT:'点'}[t] || t || '');
}
...
function syncCanvasSize(){
  // v62: iPad横画面などでCSSサイズだけ変わり、内部canvasサイズが古いままだと
  // ブラウザがビットマップを引き伸ばして円が楕円に見える。
  // getBoundingClientRect()の実表示サイズに内部解像度を常に同期させる。
  ...
  // 描画座標はCSS px基準で統一する。X/Yで別倍率は使わない。
...
// v80: 起動時にレイヤー0を必ず保証する安定化処理
function ensureLayerZeroV691(){
  try {
    if (typeof layerVisible !== 'undefined' && layerVisible instanceof Map && !layerVisible.has('0')) layerVisible.set('0', true);
    if (typeof layerLocked !== 'undefined' && layerLocked instanceof Map && !layerLocked.has('0')) layerLocked.set('0', false);
    if (typeof layerColors !== 'undefined' && layerColors instanceof Map && !layerColors.has('0')) layerColors.set('0', 7);
    if (typeof currentDrawLayer !== 'undefined' && (!currentDrawLayer || currentDrawLayer === '')) currentDrawLayer = '0';
    if (typeof currentLayerSelect !== 'undefined' && currentLayerSelect && !Array.from(currentLayerSelect.options).some(o=>o.value==='0')) {
      const opt=document.createElement('option');
      opt.value='0';
      opt.textContent='0';
      currentLayerSelect.insertBefore(opt, currentLayerSelect.firstChild);
      currentLayerSelect.value='0';
    }
  } catch(e) {
    console.warn('ensureLayerZeroV691 failed', e);
  }
}

ensureLayerZeroV691();

// v80: 安定版 現在色のみ追加（レイヤーパネルは触らない）
const ACI_COLORS_V691 = {
  BYLAYER:{label:'BYLAYER', css:null, aci:null},
  1:{label:'赤', css:'#ff0000', aci:1},
  2:{label:'黄', css:'#ffff00', aci:2},
  3:{label:'緑', css:'#00ff00', aci:3},
  4:{label:'シアン', css:'#00ffff', aci:4},
  5:{label:'青', css:'#0066ff', aci:5},
  6:{label:'マゼンタ', css:'#ff00ff', aci:6},
  7:{label:'白', css:'#ffffff', aci:7},
  8:{label:'濃グレー', css:'#666666', aci:8},
  9:{label:'薄グレー', css:'#aaaaaa', aci:9}
};
function colorKeyV691(v){
  if(v === undefined || v === null || v === '' || String(v).toUpperCase()==='BYLAYER') return 'BYLAYER';
  const n = Number(v);
  return Number.isFinite(n) && ACI_COLORS_V691[n] ? String(n) : 'BYLAYER';
}
function colorLabelV691(v){ return ACI_COLORS_V691[colorKeyV691(v)]?.label || 'BYLAYER'; }
function colorCssV691(v, fallback='#ffffff'){
  const k=colorKeyV691(v);
  if(k==='BYLAYER') return fallback;
  return ACI_COLORS_V691[k]?.css || fallback;
}
function makeColorChipV691(value){
  const k=colorKeyV691(value);
  const chip=document.createElement('span');
  chip.className='colorChipV691';
  if(k==='BYLAYER') chip.classList.add('bylayer');
  else chip.style.backgroundColor=colorCssV691(k);
  return chip;
}
function makeCurrentColorSelectV691(){
  const wrap=document.createElement('span');
  wrap.className='currentColorControlV691';
  const chip=makeColorChipV691(window.currentDrawColorV691 || 'BYLAYER');
  const sel=document.createElement('select');
  ['BYLAYER','1','2','3','4','5','6','7','8','9'].forEach(k=>{
    const opt=document.createElement('option');
    opt.value=k;
    opt.textContent=colorLabelV691(k);
    opt.selected=(colorKeyV691(window.currentDrawColorV691 || 'BYLAYER')===k);
    sel.appendChild(opt);
  });
  sel.onchange=()=>{
    window.currentDrawColorV691 = sel.value;
    localStorage.setItem('surveycad.currentColorV691', sel.value);
    const newChip=makeColorChipV691(sel.value);
    chip.replaceWith(newChip);
    command(`現在色：${colorLabelV691(sel.value)}`);
  };
  wrap.append(chip, sel);
  return wrap;
}
function initCurrentColorToolbarV691(){
  if(document.getElementById('currentColorControlV691')) return;
  window.currentDrawColorV691 = localStorage.getItem('surveycad.currentColorV691') || 'BYLAYER';
  const layerSel=document.getElementById('currentLayerSelect');
  if(!layerSel || !layerSel.parentElement) return;
  const label=document.createElement('label');
  label.id='currentColorControlV691';
  label.className='currentColorToolbarV691';
  const t=document.createElement('span');
  t.textContent='色';
  label.append(t, makeCurrentColorSelectV691());
  layerSel.parentElement.insertAdjacentElement('afterend', label);
}
setTimeout(()=>{ ensureLayerZeroV691(); initCurrentColorToolbarV691(); }, 150);

function setMode(m){
  mode=m; Object.entries(buttons).forEach(([k,b])=>{ if(b) b.classList.toggle('active', k===m); });
  ...
  if(m !== 'select' && m !== 'pan' && m !== 'trim') clearSelection();
  updateProperties();
  updateDrawHint();
  if(m === 'trim'){
    clearSelection();
    trimState = {phase:'boundary', boundaries:[]};
    command('TRIM\n境界を選択してください。複数選択できます。\n何もないところをクリックすると、消したい側をクリックするモードへ進みます。');
  }
  draw();
}
...
modeLine.onclick=()=>setMode('line'); modePoly.onclick=()=>setMode('poly'); modeCircle.onclick=()=>setMode('circle'); modeText.onclick=()=>setMode('text'); if(typeof modeOffset !== 'undefined') modeOffset.onclick=()=>setMode('offset'); if(typeof modeTrim !== 'undefined') modeTrim.onclick=()=>setMode('trim');
...
function addEntity(e){
  // v80ColorApplied: 新規作図は現在色を継承
  try{ if(e && (e.color === undefined || e.color === null || e.color === '')) e.color = getCurrentDrawColorForEntityV80(); }catch(e){}
  // v691ColorApplied: 新規作図は現在色を継承
  try{ if(e && e.color === undefined) e.color = (window.currentDrawColorV691 || 'BYLAYER'); }catch(e){}
  if(e && !e.layer) e.layer = currentDrawLayer;
  applyCurrentColorToEntity(e);
...
if(currentColorSelect){ currentColorSelect.addEventListener('change', ()=>setCurrentDrawColor(currentColorSelect.value)); updateCurrentColorUI(); }
...
// v80: レイヤー色項目を安全に追加
function ensureLayerColorsV692(){
  try{
    if(typeof layerColors === 'undefined') window.layerColors = window.layerColors || new Map();
    const map = (typeof layerColors !== 'undefined') ? layerColors : window.layerColors;
    if(map instanceof Map && !map.has('0')) map.set('0', 7);
    if(typeof allLayers === 'function') allLayers().forEach(l=>{ if(map instanceof Map && !map.has(l)) map.set(l, 7); });
  }catch(e){}
}
function getLayerColorV692(layerName){
  ensureLayerColorsV692();
  try{ const map=(typeof layerColors !== 'undefined') ? layerColors : window.layerColors; if(map instanceof Map && map.has(layerName)) return map.get(layerName); }catch(e){}
  return 7;
}
function setLayerColorV692(layerName, value){
  ensureLayerColorsV692();
  try{ const map=(typeof layerColors !== 'undefined') ? layerColors : window.layerColors; if(map instanceof Map) map.set(layerName, Number(value)); }catch(e){}
}
function makeLayerColorSelectV692(layerName){
  const wrap=document.createElement('span');
  wrap.className='currentColorControlV691 layerColorControlV692';
  const value=getLayerColorV692(layerName);
  const chip=makeColorChipV691(value);
  const sel=document.createElement('select');
  ['1','2','3','4','5','6','7','8','9'].forEach(k=>{
    const opt=document.createElement('option'); opt.value=k; opt.textContent=colorLabelV691(k); opt.selected=(colorKeyV691(value)===k); sel.appendChild(opt);
  });
  sel.onchange=()=>{
    const v=Number(sel.value); setLayerColorV692(layerName,v);
    chip.replaceWith(makeColorChipV691(v));
    command(`LAYER\nレイヤー「${layerName}」の色を変更：${colorLabelV691(v)}`);
    draw();
  };
  wrap.append(chip, sel);
  return wrap;
}
function decorateLayerPanelColorsV692(){
  ensureLayerColorsV692();
  const panel=document.querySelector('.layerPanel');
  if(!panel) return;
  const layers=(typeof allLayers==='function')?allLayers():['0'];
  layers.forEach(layerName=>{
    if(panel.querySelector(`[data-layer-color-v692="${layerName}"]`)) return;
    const rows=Array.from(panel.querySelectorAll('div, li, tr, button, label'));
    const row=rows.find(el=>{
      const txt=(el.textContent||'').trim();
      return txt===layerName || txt.endsWith(' '+layerName) || txt.includes(layerName);
    });
    if(!row) return;
    const holder=document.createElement('span');
    holder.className='layerColorHolderV692';
    holder.dataset.layerColorV692=layerName;
    holder.appendChild(makeLayerColorSelectV692(layerName));
    row.appendChild(holder);
  });
}
(function(){
  setTimeout(decorateLayerPanelColorsV692,300);
  document.addEventListener('click', e=>{ if(e.target && e.target.closest('.layerPanel')) setTimeout(decorateLayerPanelColorsV692,0); }, true);
  document.addEventListener('change', e=>{ if(e.target && e.target.closest('.layerPanel')) setTimeout(decorateLayerPanelColorsV692,0); }, true);
})();
...
function updateProperties(){
  ...
  if(selected.length > 1){
    ...
    const counts = {};
    for(const idx of selected){ const t = displayEntityType(entities[idx]?.type || 'UNKNOWN'); counts[t] = (counts[t]||0)+1; }
...
const colorPickerV70 = makeColorChipPickerV70(e.color === undefined ? 'BYLAYER' : e.color, v => {
    if(!ensureEditableEntity(e, '色変更')){ updateProperties(); return; }
    if(v==='BYLAYER') delete e.color; else e.color=Number(v);
    refreshGeometry(); commitHistory();
    command(`色を変更：${colorLabelV691(v)}`);
    draw(); updateProperties();
  }, true);
  addPropNode('色', colorPickerV70);
  addProp('種類', displayEntityType(e.type));
  if(e.type==='LINE'){
...
function parseDXF(text){
  ...
  for(let i=0;i<p.length;i++){
    ...
    const colorCode = parseInt(rec.find(x=>x.code==='62')?.val || '256', 10);
    const entColor = Number.isFinite(colorCode) && colorCode !== 256 ? colorCode : undefined;
    if(type==='LINE'){
      const x1=num(rec,'10'), y1=num(rec,'20'), x2=num(rec,'11'), y2=num(rec,'21');
      if(valid(x1,y1,x2,y2)){ const e={type, layer, pts:[{x:x1,y:y1},{x:x2,y:y2}]}; if(entColor) e.color=entColor; out.push(e); }
      ...
      if(pts.length>1){ const e={type, layer, pts, closed: (num(rec,'70') & 1)===1}; if(entColor) e.color=entColor; out.push(e); }
    } else if(type==='POINT'){
      const x=num(rec,'10'), y=num(rec,'20'); if(valid(x,y)){ const e={type, layer, pts:[{x,y}]}; if(entColor) e.color=entColor; out.push(e); }
    } else if(type==='CIRCLE'){
      const x=num(rec,'10'), y=num(rec,'20'), r=num(rec,'40'); if(valid(x,y,r)){ const e={type, layer, center:{x,y}, r, pts:[{x,y}]}; if(entColor) e.color=entColor; out.push(e); }
    } else if(type==='ARC'){
      const x=num(rec,'10'), y=num(rec,'20'), r=num(rec,'40'), a1=num(rec,'50'), a2=num(rec,'51'); if(valid(x,y,r,a1,a2)){ const e={type, layer, center:{x,y}, r, a1, a2, pts:[{x,y}]}; if(entColor) e.color=entColor; out.push(e); }
    } else if(type==='TEXT' || type==='MTEXT'){
      const x=num(rec,'10'), y=num(rec,'20'); const textVal=rec.find(a=>a.code==='1')?.val || '';
      const h=num(rec,'40'); if(valid(x,y)){ const e={type:'TEXT', layer, pts:[{x,y}], text:textVal, height:Number.isFinite(h)?h:2.5}; if(entColor) e.color=entColor; out.push(e); }
    }
  }
  return out;
}
...
canvas.addEventListener('pointerup', e=>{
  if(lineReleaseState && lineReleaseState.pointerId === e.pointerId){
    ...
    if(p1 && p2){
      addEntity({type:'LINE', layer:currentDrawLayer, pts:[{x:p1.x,y:p1.y},{x:p2.x,y:p2.y}], created:true, color:(typeof e!=='undefined' && e && e.color!==undefined)?e.color:getCurrentDrawColorForEntityV80()});
  ...
  if(circleReleaseState && circleReleaseState.pointerId === e.pointerId){
    ...
    if(c && pR){
      const rr = Math.hypot(pR.x-c.x, pR.y-c.y);
      if(rr>0){
        addEntity({type:'CIRCLE', layer:currentDrawLayer, center:{x:c.x,y:c.y}, r:rr, pts:[{x:c.x,y:c.y}], created:true, color:(typeof e!=='undefined' && e && e.color!==undefined)?e.color:getCurrentDrawColorForEntityV80()});
...
function handleTapAt(clientX, clientY){
  ...
  // v80.1: TRIMモード
  if(mode==='trim' || trimState){ handleTrimTap(raw, p); return; }
  // v62: OFFSETモードでは、offsetState が null の最初のタップでも対象選択へ入る。
  if(mode==='offset' || offsetState){ handleOffsetTap(raw, p); return; }
...
function updateDrawHint(extra=''){
  ...
  else if(mode==='offset') info.textContent = extra || `OFFSET\n現在レイヤ：${currentDrawLayer}${isLayerLocked(currentDrawLayer)?'（ロック中）':''}\nオフセットする LINE / POLYLINE / CIRCLE を選択`;
...
続けて押したまま探す、または「確定」`);
  ...
  if(mode==='circle'){
    ...
    if(rr>0){ addEntity({type:'CIRCLE', layer:currentDrawLayer, center:{x:c.x,y:c.y}, r:rr, pts:[{x:c.x,y:c.y}], created:true, color:(typeof e!=='undefined' && e && e.color!==undefined)?e.color:getCurrentDrawColorForEntityV80()}); }
  ...
  if(mode==='text'){
    ...
    if(t!==null && t.trim()!==''){
      addEntity({type:'TEXT', layer:currentDrawLayer, pts:[{x:p.x,y:p.y}], text:t.trim(), height:2.5, created:true, color:(typeof e!=='undefined' && e && e.color!==undefined)?e.color:getCurrentDrawColorForEntityV80()});
...
function finishDrawing(){
  if(mode==='poly'){
    ...
    if(drawState && drawState.type==='LWPOLYLINE' && drawState.pts.length>=2){
      ...
      addEntity({type:'LWPOLYLINE', layer:currentDrawLayer, pts, closed:false, created:true, color:(typeof e!=='undefined' && e && e.color!==undefined)?e.color:getCurrentDrawColorForEntityV80()});
...
function cancelDrawing(){
  hideCircleChoicePanel();
  drawState=null; lineReleaseState=null; polyReleaseState=null; circleReleaseState=null; measureStart=null; measureEnd=null; moveState=null; copyState=null; offsetState=null; trimState=null; hideObjectMenu();
...
function exportDXF(es){
  ...
  for(const e of es){
    ...
    if(e.type==='LINE'){
      add(0,'LINE'); add(8,layer); addColorDXF(add,e); add(10,fmt(e.pts[0].x)); add(20,fmt(e.pts[0].y)); add(30,0); add(11,fmt(e.pts[1].x)); add(21,fmt(e.pts[1].y)); add(31,0);
    } else if(e.type==='LWPOLYLINE'){
      add(0,'LWPOLYLINE'); add(8,layer); addColorDXF(add,e); add(90,e.pts.length); add(70,e.closed?1:0);
      for(const p of e.pts){ add(10,fmt(p.x)); add(20,fmt(p.y)); }
    } else if(e.type==='POINT'){
      add(0,'POINT'); add(8,layer); addColorDXF(add,e); add(10,fmt(e.pts[0].x)); add(20,fmt(e.pts[0].y)); add(30,0);
    } else if(e.type==='CIRCLE'){
      add(0,'CIRCLE'); add(8,layer); addColorDXF(add,e); add(10,fmt(e.center.x)); add(20,fmt(e.center.y)); add(30,0); add(40,fmt(e.r));
    } else if(e.type==='ARC'){
      add(0,'ARC'); add(8,layer); addColorDXF(add,e); add(10,fmt(e.center.x)); add(20,fmt(e.center.y)); add(30,0); add(40,fmt(e.r)); add(50,fmt(e.a1)); add(51,fmt(e.a2));
    } else if(e.type==='TEXT'){
      add(0,'TEXT'); add(8,layer); addColorDXF(add,e); add(10,fmt(e.pts[0].x)); add(20,fmt(e.pts[0].y)); add(30,0); add(40,fmt(e.height || 2.5)); add(1,escapeDXFText(e.text || ''));
    }
  }
...
function entityTypeLabel(e){
  if(!e) return '';
  const map={LINE:'線分', LWPOLYLINE:'ポリライン', CIRCLE:'円', ARC:'円弧', TEXT:'文字', POINT:'点'};
  return map[e.type] || e.type;
}
function boundarySegmentsFromEntity(e){
  const out=[];
  if(!e) return out;
  if(e.type==='LINE' && e.pts && e.pts.length>=2){
    out.push({a:e.pts[0], b:e.pts[1]});
  } else if(e.type==='LWPOLYLINE' && e.pts && e.pts.length>=2){
    for(let i=0;i<e.pts.length-1;i++) out.push({a:e.pts[i], b:e.pts[i+1]});
    if(e.closed && e.pts.length>2) out.push({a:e.pts[e.pts.length-1], b:e.pts[0]});
  }
  return out;
}
function getTrimBoundarySegments(){
  if(!trimState || !trimState.boundaries) return [];
  const segs=[];
  for(const idx of trimState.boundaries){
    const e=entities[idx];
    boundarySegmentsFromEntity(e).forEach(seg=>segs.push(seg));
  }
  return segs;
}
function paramOnSegment(p,a,b){
  ...
  return ((p.x-a.x)*vx + (p.y-a.y)*vy) / len2;
}
function pointAtParam(a,b,t){
  return {x:a.x+(b.x-a.x)*t, y:a.y+(b.y-a.y)*t};
}
function trimLineEntityAtPoint(index, clickPoint, boundarySegs){
  const e=entities[index];
  if(!e || e.type!=='LINE' || !e.pts || e.pts.length<2) return false;
  const a=e.pts[0], b=e.pts[1];
  const ts=[];
  for(const seg of boundarySegs){
    const ip=lineIntersection(a,b,seg.a,seg.b);
    if(ip){
      const t=paramOnSegment(ip,a,b);
      if(t>1e-8 && t<1-1e-8 && !ts.some(v=>Math.abs(v-t)<1e-7)) ts.push(t);
    }
  }
  ts.sort((x,y)=>x-y);
  if(ts.length===0){
  ...
  const tc=Math.max(0, Math.min(1, paramOnSegment(clickPoint,a,b)));
  const cuts=[0, ...ts, 1];
  let removeInterval=-1;
  for(let i=0;i<cuts.length-1;i++){
    if(tc>=cuts[i]-1e-9 && tc<=cuts[i+1]+1e-9){ removeInterval=i; break; }
  }
  if(removeInterval<0) return false;
  const remain=[];
  for(let i=0;i<cuts.length-1;i++){
    if(i===removeInterval) continue;
    const t0=cuts[i], t1=cuts[i+1];
    if(t1-t0 < 1e-8) continue;
    remain.push({type:'LINE', layer:e.layer || currentDrawLayer || '0', pts:[pointAtParam(a,b,t0), pointAtParam(a,b,t1)], created:true, color:e.color});
  }
  // v80.1: TRIM対象と境界ハイライトを分離。
  // 黄色表示は境界オブジェクト専用。トリム対象を黄色選択に入れない。
  if(remain.length===0){
    entities.splice(index,1);
    // 削除により、削除位置より後ろの境界インデックスは1つ前へずれる。
    if(trimState && Array.isArray(trimState.boundaries)){
      trimState.boundaries = trimState.boundaries
        .filter(i => i !== index)
        .map(i => i > index ? i - 1 : i);
    }
  } else {
    entities[index]=remain[0];
    for(let i=1;i<remain.length;i++) entities.push(remain[i]);
  }
  refreshGeometry();
  commitHistory();

  // ここが重要：トリム後の選択はトリム対象ではなく、境界だけに戻す。
  if(trimState && Array.isArray(trimState.boundaries)){
    setSelection(trimState.boundaries);
  } else {
    clearSelection();
  }
  updateProperties();
  command(`TRIM\n線分をトリムしました。\n境界数：${trimState && trimState.boundaries ? trimState.boundaries.length : 0}\n続けて消したい側をクリックできます。`);
  draw();
  return true;
}
function handleTrimTap(raw, snappedPoint){
  if(!trimState) trimState = {phase:'boundary', boundaries:[]};
  if(trimState.phase === 'boundary'){
    const hit = hitTestEntity(raw, 14/view.scale);
    if(hit && entities[hit.index] && ['LINE','LWPOLYLINE'].includes(entities[hit.index].type)){
      if(!trimState.boundaries.includes(hit.index)) trimState.boundaries.push(hit.index);
      setSelection(trimState.boundaries);
      updateProperties();
      command(`TRIM\n境界を選択中：${trimState.boundaries.length}個\n追加選択できます。\n何もないところをクリックすると、消したい側をクリックするモードへ進みます。`);
      draw();
      return;
    }
    if(trimState.boundaries.length > 0){
      trimState.phase = 'cut';
      setSelection(trimState.boundaries);
      command(`TRIM\n境界選択完了：${trimState.boundaries.length}個\n消したい側の線分をクリックしてください。\n終了は取消ボタン、別モード、または再度TRIM。`);
      draw();
      return;
    }
    command('TRIM\n境界にする線分またはポリラインを選択してください。');
    draw();
    return;
  }
  if(trimState.phase === 'cut'){
    const hit = hitTestEntity(raw, 14/view.scale);
    if(!hit || !entities[hit.index]){
      command('TRIM\n消したい側の線分をクリックしてください。');
      draw();
      return;
    }
    if(trimState.boundaries.includes(hit.index)){
      command('TRIM\n境界オブジェクト自身はトリム対象にできません。');
      return;
    }
    const target=entities[hit.index];
    if(isEntityLocked(target)){ command(`TRIM不可：対象レイヤ「${target.layer || '0'}」はロック中です。`); return; }
    if(target.type !== 'LINE'){
      command(`TRIM\nv80.1ではトリム対象は線分のみ対応です。\n選択された種類：${entityTypeLabel(target)}\n境界には線分・ポリラインを使えます。`);
      return;
    }
    const segs=getTrimBoundarySegments();
    trimLineEntityAtPoint(hit.index, raw, segs);
    return;
  }
}
...
// v62: OFFSET command
function handleOffsetTap(raw, snappedPoint){
  if(!ensureCurrentLayerEditable('OFFSET')) return;
  if(offsetPromptPending){ command('OFFSET\n距離入力待ちです。'); return; }
  const p = snappedPoint || raw;
  if(!offsetState){
    const hit = hitTestEntity(raw, 14/view.scale);
    if(!hit || !entities[hit.index] || !['LINE','LWPOLYLINE','CIRCLE'].includes(entities[hit.index].type)){
      command('OFFSET\nオフセット可能な LINE / POLYLINE / CIRCLE を選択してください。');
      return;
    }
    const target = entities[hit.index];
    if(isEntityLocked(target)){ command(`OFFSET不可：対象レイヤ「${target.layer || '0'}」はロック中です。`); return; }

    // 対象を選んだ瞬間にSELECTと同じ黄色ハイライトを表示してから距離入力へ進む。
    setSelection([hit.index]);
    updateProperties();
    command(`OFFSET\n対象：${target.type} を選択しました。\n距離を入力してください。`);
    draw();

    offsetPromptPending = true;
    setTimeout(()=>{
      offsetPromptPending = false;
      const lastD = localStorage.getItem('surveycad.offsetDistance') || '1.000';
      const s = prompt('オフセット距離を入力してください', lastD);
      if(s === null){ command('OFFSET\nキャンセルしました。'); draw(); return; }
      const d = Number(String(s).trim());
      if(!Number.isFinite(d) || d <= 0){ command('OFFSET\n距離は0より大きい数値を入力してください。'); draw(); return; }
      localStorage.setItem('surveycad.offsetDistance', String(d));
      offsetState = {index:hit.index, source:deepCloneEntity(target), distance:d, phase:'direction', sidePoint:null};
      command(`OFFSET\n対象：${target.type}\n距離：${d.toFixed(3)}\nオフセットする方向をクリックしてください。\n※1回目はプレビュー、2回目で確定。`);
      draw();
    }, 80);
    return;
  }
  // v62: 1回目の方向クリックはプレビュー、2回目のクリックで確定。
  if(!offsetState.phase || offsetState.phase === 'direction'){
    offsetState.phase = 'confirm';
    offsetState.sidePoint = {x:raw.x, y:raw.y};
    command(`OFFSET\n仮オフセットを表示中。\n良ければ同じ方向側をもう一度クリックして確定してください。\n距離：${offsetState.distance.toFixed(3)}`);
    draw();
    return;
  }

  if(offsetState.phase === 'confirm'){
    const side = {x:raw.x, y:raw.y};
    const created = createOffsetEntity(offsetState.source, offsetState.distance, side);
    if(!created){
      command('OFFSET\nこのオブジェクトはオフセットできませんでした。');
      offsetState = null;
      draw();
      return;
    }
    created.layer = currentDrawLayer;
...
function createOffsetEntity(e, d, sidePoint){
  if(!e) return null;
  if(e.type === 'LINE'){
    const a=e.pts[0], b=e.pts[1];
    ...
    const s = cross >= 0 ? 1 : -1;
    const off={x:n.x*d*s, y:n.y*d*s};
    return {type:'LINE', layer:currentDrawLayer, pts:[{x:a.x+off.x,y:a.y+off.y},{x:b.x+off.x,y:b.y+off.y}]};
  }
  if(e.type === 'CIRCLE'){
    const rr0 = e.r || 0;
    const dc = Math.hypot(sidePoint.x-e.center.x, sidePoint.y-e.center.y);
    const rr = dc >= rr0 ? rr0 + d : rr0 - d;
    if(rr <= 1e-9){ command('OFFSET\n円の内側オフセット半径が0以下になります。'); return null; }
    return {type:'CIRCLE', layer:currentDrawLayer, center:{x:e.center.x,y:e.center.y}, r:rr, pts:[{x:e.center.x,y:e.center.y}]};
  }
  if(e.type === 'LWPOLYLINE'){
    const plus = offsetPolylineEntity(e, d);
    const minus = offsetPolylineEntity(e, -d);
    if(!plus && !minus) return null;
    if(!plus) return minus;
    if(!minus) return plus;
    const dp = distancePointToEntity(sidePoint, plus);
    const dm = distancePointToEntity(sidePoint, minus);
    return dp <= dm ? plus : minus;
  }
  return null;
}

function offsetPolylineEntity(e, d){
  ...
  for(let i=0;i<segCount;i++){
    ...
    segs.push({i,a:{x:a.x+nx*d,y:a.y+ny*d}, b:{x:b.x+nx*d,y:b.y+ny*d}, vx, vy});
  }
  ...
  if(!closed){
    ...
    for(let i=0;i<segs.length-1;i++){
      out.push(intersectOffsetLines(segs[i], segs[i+1]) || averagePoint(segs[i].b, segs[i+1].a));
    }
    ...
    for(let i=0;i<segs.length;i++){
      ...
      const cur=segs[i];
      out.push(intersectOffsetLines(prev, cur) || averagePoint(prev.b, cur.a));
    }
  }
  return {type:'LWPOLYLINE', layer:currentDrawLayer, pts:out, closed};
}
function averagePoint(a,b){ return {x:(a.x+b.x)/2, y:(a.y+b.y)/2}; }
function intersectOffsetLines(s1,s2){
  const p=s1.a, r={x:s1.b.x-s1.a.x, y:s1.b.y-s1.a.y};
  const q=s2.a, u={x:s2.b.x-s2.a.x, y:s2.b.y-s2.a.y};
  const den = r.x*u.y - r.y*u.x;
  if(Math.abs(den) < 1e-12) return null;
  const t = ((q.x-p.x)*u.y - (q.y-p.y)*u.x)/den;
  return {x:p.x+t*r.x, y:p.y+t*r.y};
}
function distancePointToEntity(p,e){
  if(!e) return Infinity;
  if(e.type==='LINE') return distPt(p, closestPointOnSegment(p,e.pts[0],e.pts[1]));
  if(e.type==='LWPOLYLINE'){
    let best=Infinity;
    for(let i=0;i<e.pts.length-1;i++) best=Math.min(best, distPt(p, closestPointOnSegment(p,e.pts[i],e.pts[i+1])));
    if(e.closed && e.pts.length>1) best=Math.min(best, distPt(p, closestPointOnSegment(p,e.pts[e.pts.length-1],e.pts[0])));
    return best;
  }
  if(e.type==='CIRCLE') return Math.abs(distPt(p,e.center)-e.r);
  return Infinity;
}
function drawOffsetPreview(){
  if(!offsetState || !offsetState.source) return;
  let side = null;
  if(offsetState.phase === 'confirm' && offsetState.sidePoint){
    side = offsetState.sidePoint;
  } else if(currentCursorScreen) {
    side = screenToWorld(currentCursorScreen.x, currentCursorScreen.y);
  }
  if(!side) return;
  const preview = createOffsetEntity(offsetState.source, offsetState.distance, side);
  if(!preview) return;
  ctx.save();
  ...
  ctx.lineWidth=1.4;
  ctx.setLineDash([6,4]);
  if(preview.type==='LINE' || preview.type==='LWPOLYLINE'){
    ctx.beginPath();
    preview.pts.forEach((p,i)=>{ const s=worldToScreen(p); i?ctx.lineTo(s.x,s.y):ctx.moveTo(s.x,s.y); });
    if(preview.closed) ctx.closePath();
    ctx.stroke();
  } else if(preview.type==='CIRCLE'){
    const s=worldToScreen(preview.center);
    ctx.beginPath();
    ctx.arc(s.x,s.y,Math.abs(preview.r*view.scale),0,Math.PI*2);
    ctx.stroke();
  }
  ctx.restore();
}
...
/* v80.1: TRIM boundary lock patch */
(function(){
  window.__trimLockedBoundaries = window.__trimLockedBoundaries || null;

  window.lockTrimBoundariesV661 = function(){
    if (Array.isArray(window.trimBoundaries)) {
      window.__trimLockedBoundaries = window.trimBoundaries.slice();
    }
    if (window.trimState && Array.isArray(window.trimState.boundaries)) {
      window.__trimLockedBoundaries = window.trimState.boundaries.slice();
    }
  };

  window.restoreTrimBoundariesV661 = function(){
    if (!window.__trimLockedBoundaries) return;
    if (Array.isArray(window.trimBoundaries)) {
      window.trimBoundaries = window.__trimLockedBoundaries.slice();
    }
    if (window.trimState && Array.isArray(window.trimState.boundaries)) {
      window.trimState.boundaries = window.__trimLockedBoundaries.slice();
    }
  };

  ["finishTrimBoundarySelection","finishTrimBoundaries","completeTrimBoundarySelection"].forEach(function(name){
    const old = window[name];
    if (typeof old === "function") {
      window[name] = function(){
        const r = old.apply(this, arguments);
        window.lockTrimBoundariesV661();
        return r;
      };
    }
  });

  ["applyTrim","trimEntity","performTrim","executeTrim"].forEach(function(name){
    const old = window[name];
    if (typeof old === "function") {
      window[name] = function(){
        const saved = window.__trimLockedBoundaries ? window.__trimLockedBoundaries.slice() : null;
        const r = old.apply(this, arguments);
        if (saved) {
          window.__trimLockedBoundaries = saved;
          window.restoreTrimBoundariesV661();
        }
        return r;
      };
    }
...
/* SurveyCAD v80.1
   TRIM表示ルール修正:
   - 黄色ハイライトは境界オブジェクト専用
   - トリム対象は selectedEntities に入れない
   - トリム対象用の一時変数 trimTargetPreview を用意
   - 境界選択完了後も trimBoundaries を維持
*/
(function(){
  if (window.__surveyCadTrimV662PatchApplied) return;
  window.__surveyCadTrimV662PatchApplied = true;

  window.trimTargetPreview = null;

  function getTrimPhase(){
    try {
      if (typeof trimPhase !== "undefined") return trimPhase;
      if (window.trimState && window.trimState.phase) return window.trimState.phase;
      if (typeof trimMode !== "undefined") return trimMode;
    } catch(e) {}
    return null;
  }

  function inTrimCutPhase(){
    const p = String(getTrimPhase() || "").toLowerCase();
    return p.includes("cut") || p.includes("execute") || p.includes("target") || p.includes("trim");
  }

  function getTrimBoundaries(){
    try {
      if (typeof trimBoundaries !== "undefined" && Array.isArray(trimBoundaries)) return trimBoundaries;
      if (window.trimBoundaries && Array.isArray(window.trimBoundaries)) return window.trimBoundaries;
      if (window.trimState && Array.isArray(window.trimState.boundaries)) return window.trimState.boundaries;
    } catch(e) {}
    return null;
  }

  function boundaryIds(){
    const bs = getTrimBoundaries() || [];
    return bs.map(e => e && (e.id ?? e._id ?? e.uid)).filter(v => v != null);
  }

  function restoreYellowToBoundariesOnly(){
    const bs = getTrimBoundaries();
    if (!bs || !bs.length) return;
    try {
      if (typeof selectedEntities !== "undefined" && Array.isArray(selectedEntities)) {
        selectedEntities = bs.slice();
      }
    } catch(e) {}
    try {
      if (window.selectedEntities && Array.isArray(window.selectedEntities)) {
        window.selectedEntities = bs.slice();
      }
    } catch(e) {}
    try {
  ...
  // Wrap common trim execution functions so target never remains yellow after trim.
  ["applyTrim","performTrim","executeTrim","trimEntity","trimLineByBoundaries"].forEach(function(name){
    const old = window[name];
    if (typeof old === "function" && !old.__v662Wrapped) {
      const wrapped = function(){
        const r = old.apply(this, arguments);
        window.trimTargetPreview = null;
        restoreYellowToBoundariesOnly();
        if (typeof render === "function") render();
        else if (typeof draw === "function") draw();
        return r;
      };
      wrapped.__v662Wrapped = true;
      window[name] = wrapped;
    }
  });

  // Wrap render/draw to clean accidental yellow target during TRIM cut phase.
  ["render","draw","redraw"].forEach(function(name){
    const old = window[name];
    if (typeof old === "function" && !old.__v662Wrapped) {
      const wrapped = function(){
        if (inTrimCutPhase()) restoreYellowToBoundariesOnly();
  ...
  // Expose helper for existing handlers if needed.
  window.restoreTrimYellowToBoundariesOnlyV662 = restoreYellowToBoundariesOnly;
})();




/* SurveyCAD v80.1: TRIM中は黄色選択を境界だけに戻す */
(function(){
  if(window.__trimV663Applied) return;
  window.__trimV663Applied = true;
  window.__trimBoundaryHighlight = window.__trimBoundaryHighlight || [];

  function getBoundaries(){
    try { if(Array.isArray(window.trimBoundaries) && window.trimBoundaries.length) return window.trimBoundaries; } catch(e){}
    try { if(window.trimState && Array.isArray(window.trimState.boundaries) && window.trimState.boundaries.length) return window.trimState.boundaries; } catch(e){}
    try { if(typeof trimBoundaries !== "undefined" && Array.isArray(trimBoundaries) && trimBoundaries.length) return trimBoundaries; } catch(e){}
    try { if(typeof trimState !== "undefined" && trimState && Array.isArray(trimState.boundaries) && trimState.boundaries.length) return trimState.boundaries; } catch(e){}
    return window.__trimBoundaryHighlight || [];
  }

  function trimActive(){
    let s="";
    try { if(window.currentMode) s += " " + window.currentMode; } catch(e){}
    try { if(window.mode) s += " " + window.mode; } catch(e){}
    try { if(window.command) s += " " + window.command; } catch(e){}
    try { if(typeof currentMode !== "undefined") s += " " + currentMode; } catch(e){}
    try { if(typeof mode !== "undefined") s += " " + mode; } catch(e){}
    try { if(typeof command !== "undefined") s += " " + command; } catch(e){}
    try { if(window.trimState && window.trimState.phase) s += " " + window.trimState.phase; } catch(e){}
    try { if(typeof trimPhase !== "undefined") s += " " + trimPhase; } catch(e){}
    return /trim/i.test(s) || (window.__trimBoundaryHighlight && window.__trimBoundaryHighlight.length);
  }
  ...
  window.trimSaveBoundariesV663 = function(){
    const b = unique(getBoundaries());
    if(b.length) window.__trimBoundaryHighlight = b.slice();
  };

  window.trimRestoreYellowV663 = function(){
    if(!trimActive()) return;
    window.trimSaveBoundariesV663();
    const b = unique(window.__trimBoundaryHighlight);
    if(!b.length) return;
    try { window.selectedEntities = b.slice(); } catch(e){}
    try { if(typeof selectedEntities !== "undefined") selectedEntities = b.slice(); } catch(e){}
    try { window.selectedEntity = null; } catch(e){}
    try { if(typeof selectedEntity !== "undefined") selectedEntity = null; } catch(e){}
  };

  ["finishTrimBoundarySelection","finishTrimBoundaries","completeTrimBoundarySelection","applyTrim","performTrim","executeTrim","trimEntity","trimLineByBoundaries"].forEach(name=>{
    const old = window[name];
    if(typeof old === "function" && !old.__v663){
      const wrapped = function(){
        window.trimSaveBoundariesV663();
        const saved = window.__trimBoundaryHighlight.slice();
        const r = old.apply(this, arguments);
        if(saved.length) window.__trimBoundaryHighlight = saved;
        window.trimRestoreYellowV663();
        return r;
      };
      wrapped.__v663 = true;
      window[name] = wrapped;
  ...
  ["render","draw","redraw"].forEach(name=>{
    const old = window[name];
    if(typeof old === "function" && !old.__v663){
      const wrapped = function(){
        if(trimActive()) window.trimRestoreYellowV663();
        return old.apply(this, arguments);
      };
      wrapped.__v663 = true;
      window[name] = wrapped;
...
/* SurveyCAD v80.1: TRIM本体修正済み。黄色は境界オブジェクト専用。 */


// v80: 色名表示をやめ、色チップだけで選択するUI
function colorPaletteKeysV70(includeByLayer=true){
  const base = ['1','2','3','4','5','6','7','8','9'];
  return includeByLayer ? ['BYLAYER', ...base] : base;
}
function makeChipButtonV70(key, selected, onpick){
  const btn=document.createElement('button');
  btn.type='button';
  btn.className='colorChipButtonV70' + (selected ? ' selected' : '');
  btn.title=colorLabelV691(key);
  btn.appendChild(makeColorChipV691(key));
  btn.onclick=(e)=>{ e.preventDefault(); e.stopPropagation(); onpick(key); };
  return btn;
}
function makeColorChipPickerV70(value, onpick, includeByLayer=true){
  const wrap=document.createElement('span');
  wrap.className='colorChipPickerV70';
  const current=makeChipButtonV70(value, false, ()=>{});
  current.classList.add('current');
  const pop=document.createElement('span');
  pop.className='colorPaletteV70 hidden';
  colorPaletteKeysV70(includeByLayer).forEach(k=>{
    pop.appendChild(makeChipButtonV70(k, colorKeyV691(value)===k, (picked)=>{
      const val = picked==='BYLAYER' ? 'BYLAYER' : Number(picked);
      onpick(val);
      const next=makeColorChipButtonCurrentV70(val, pop, onpick, includeByLayer);
      current.replaceWith(next);
      pop.classList.add('hidden');
    }));
  });
  current.onclick=(e)=>{ e.preventDefault(); e.stopPropagation(); pop.classList.toggle('hidden'); };
  wrap.append(current,pop);
  return wrap;
}
function makeColorChipButtonCurrentV70(value, pop, onpick, includeByLayer){
  const btn=makeChipButtonV70(value,false,()=>{});
  btn.classList.add('current');
  btn.onclick=(e)=>{ e.preventDefault(); e.stopPropagation(); pop.classList.toggle('hidden'); };
  return btn;
}
document.addEventListener('click', e=>{
  document.querySelectorAll('.colorPaletteV70').forEach(p=>{
    if(!p.parentElement || !p.parentElement.contains(e.target)) p.classList.add('hidden');
  });
}, true);

// v80: レイヤー色は全レイヤー初期値White(ACI 7)
function ensureLayerColorsV70(){
  try{
    if(typeof layerColors === 'undefined') window.layerColors = window.layerColors || new Map();
    const map=(typeof layerColors !== 'undefined') ? layerColors : window.layerColors;
    if(typeof allLayers === 'function') allLayers().forEach(l=>{ if(map instanceof Map && !map.has(l)) map.set(l, 7); });
...
function getLayerColorV70(layerName){
  ensureLayerColorsV70();
  try{ const map=(typeof layerColors !== 'undefined') ? layerColors : window.layerColors; if(map instanceof Map && map.has(layerName)) return map.get(layerName); }catch(e){}
  return 7;
}
function setLayerColorV70(layerName, value){
  ensureLayerColorsV70();
  try{ const map=(typeof layerColors !== 'undefined') ? layerColors : window.layerColors; if(map instanceof Map) map.set(layerName, Number(value)); }catch(e){}
}
function effectiveColorV70(e){
  const c=e && e.color!==undefined ? e.color : 'BYLAYER';
  if(colorKeyV691(c)!=='BYLAYER') return Number(c);
  return getLayerColorV70(e && e.layer ? e.layer : '0');
}

// v80: 現在色ツールバーをチップのみへ置換
function rebuildCurrentColorToolbarV70(){
  const old=document.getElementById('currentColorControlV691');
  if(old) old.remove();
  if(document.getElementById('currentColorControlV70')) return;
  window.currentDrawColorV691 = localStorage.getItem('surveycad.currentColorV691') || 'BYLAYER';
  const layerSel=document.getElementById('currentLayerSelect');
  if(!layerSel || !layerSel.parentElement) return;
  const label=document.createElement('label');
  label.id='currentColorControlV70';
  label.className='currentColorToolbarV70';
  const t=document.createElement('span');
  t.textContent='色';
  const picker=makeColorChipPickerV70(window.currentDrawColorV691, (v)=>{
    window.currentDrawColorV691 = v==='BYLAYER' ? 'BYLAYER' : String(v);
    localStorage.setItem('surveycad.currentColorV691', window.currentDrawColorV691);
    command(`現在色：${colorLabelV691(window.currentDrawColorV691)}`);
  }, true);
  label.append(t,picker);
  layerSel.parentElement.insertAdjacentElement('afterend', label);
}

// v80: レイヤーパネル色をチップのみへ置換
function decorateLayerPanelColorsV70(){
  ensureLayerColorsV70();
  const panel=document.querySelector('.layerPanel');
  if(!panel) return;
  // 古いv80のselect式色UIは消す
  panel.querySelectorAll('.layerColorHolderV692').forEach(e=>e.remove());
  const layers=(typeof allLayers==='function')?allLayers():['0'];
  layers.forEach(layerName=>{
    if(panel.querySelector(`[data-layer-color-v80="${layerName}"]`)) return;
    const rows=Array.from(panel.querySelectorAll('div, li, tr, button, label'));
    const row=rows.find(el=>{
      const txt=(el.textContent||'').trim();
      return txt===layerName || txt.endsWith(' '+layerName) || txt.includes(layerName);
    ...
    if(!row) return;
    const holder=document.createElement('span');
    holder.className='layerColorHolderV70';
    holder.dataset.layerColorV70=layerName;
    holder.appendChild(makeColorChipPickerV70(getLayerColorV70(layerName), (v)=>{
      setLayerColorV70(layerName, v==='BYLAYER'?7:v);
      command(`LAYER\nレイヤー「${layerName}」の色を変更：${colorLabelV691(v)}`);
      draw();
      setTimeout(decorateLayerPanelColorsV70,0);
    }, false));
    row.appendChild(holder);
  });
}

// v80: プロパティ内のレイヤー名・色をチップ式で編集
function rebuildPropertyLayerAndColorV70(){
  // updateProperties本体に組み込むのが理想だが、既存DOM更新後に重複を消す
  const p=document.querySelector('.propPanel');
  if(!p) return;
}
setTimeout(()=>{ ensureLayerColorsV70(); rebuildCurrentColorToolbarV70(); decorateLayerPanelColorsV70(); }, 250);
document.addEventListener('click', e=>{ if(e.target && e.target.closest('.layerPanel')) setTimeout(decorateLayerPanelColorsV70,0); }, true);



// v80: レイヤー色チップを 👁️ と 🔓 の間へ配置
function decorateLayerPanelColorsV71(){
  ensureLayerColorsV70();
  const panel=document.querySelector('.layerPanel');
  if(!panel) return;

  panel.querySelectorAll('.layerColorHolderV70').forEach(e=>e.remove());

  const layers=(typeof allLayers==='function') ? allLayers() : ['0'];
  layers.forEach(layerName=>{
    if(panel.querySelector(`[data-layer-color-inline-v80="${layerName}"]`)) return;
    ...
    const holder=document.createElement('span');
    holder.className='layerColorChipInlineV71';
    holder.dataset.layerColorInlineV71=layerName;
    holder.appendChild(makeColorChipPickerV70(getLayerColorV70(layerName), (v)=>{
      setLayerColorV70(layerName, v==='BYLAYER'?7:v);
      command(`LAYER\nレイヤー「${layerName}」の色を変更：${colorLabelV691(v)}`);
      draw();
      setTimeout(decorateLayerPanelColorsV71,0);
    }, false));

    const children=Array.from(row.children);
    let eyeEl=null, lockEl=null;

    for(const el of children){
      const txt=(el.textContent||'').trim();
      const aria=(el.getAttribute && (el.getAttribute('aria-label')||el.title||'')) || '';
      if(!eyeEl && (txt.includes('👁') || aria.includes('表示') || aria.toLowerCase().includes('visible'))) eyeEl=el;
      if(!lockEl && (txt.includes('🔓') || txt.includes('🔒') || aria.includes('ロック') || aria.toLowerCase().includes('lock'))) lockEl=el;
    }

    if(lockEl && lockEl.parentNode===row){
      row.insertBefore(holder, lockEl);
    }else if(eyeEl && eyeEl.parentNode===row && eyeEl.nextSibling){
      row.insertBefore(holder, eyeEl.nextSibling);
    }else{
      if(row.children.length>=2) row.insertBefore(holder, row.children[1]);
...
(function(){
  const run=()=>setTimeout(decorateLayerPanelColorsV71,0);
  setTimeout(decorateLayerPanelColorsV71,300);
  document.addEventListener('click', e=>{ if(e.target && e.target.closest('.layerPanel')) run(); }, true);
  document.addEventListener('change', e=>{ if(e.target && e.target.closest('.layerPanel')) run(); }, true);
})();



// v80: レイヤーパネルを 1行表示へ再構成
// 表示順: 👁️ 🔒 ■ ◉ レイヤ名
function rebuildLayerPanelRowsV72(){
  if(typeof allLayers !== 'function') return;
  const panel=document.querySelector('.layerPanel');
  if(!panel) return;

  // 既存の色チップだけが上に並んでいる状態を消す
  panel.querySelectorAll('.layerColorHolderV70,.layerColorChipInlineV71,.layerColorHolderV72').forEach(e=>e.remove());
  ...
  layers.forEach(layerName=>{
    ...
    let row=rows.find(el=>{
      if(el.classList && el.classList.contains('layerRowRebuiltV72')) return false;
      const txt=(el.textContent||'').trim();
      return txt===layerName || txt.endsWith(' '+layerName) || txt.includes(layerName);
    });
    if(!row) return;
    if(row.dataset && row.dataset.layerRowV72Done==='1') return;

    // 行の主要部品を拾う
    const children=Array.from(row.children);
    let eyeEl=null, lockEl=null, currentEl=null;
    for(const el of children){
      const txt=(el.textContent||'').trim();
      const aria=(el.getAttribute && (el.getAttribute('aria-label')||el.title||'')) || '';
      if(!eyeEl && (txt.includes('👁') || aria.includes('表示') || aria.toLowerCase().includes('visible'))) eyeEl=el;
    ...
    // 現在レイヤー白丸/チェック用の既存要素を探す
    for(const el of children){
      const txt=(el.textContent||'').trim();
      const aria=(el.getAttribute && (el.getAttribute('aria-label')||el.title||'')) || '';
      const cls=(el.className||'')+'';
      if(!currentEl && (txt==='○' || txt==='◉' || txt.includes('○') || txt.includes('◉') || aria.includes('現在') || cls.toLowerCase().includes('current'))){
        if(el!==eyeEl && el!==lockEl) currentEl=el;
      }
    }

    // なければ currentDrawLayer を切り替える丸を作る
    if(!currentEl){
      currentEl=document.createElement('button');
      currentEl.type='button';
      currentEl.className='layerCurrentMarkV72';
      currentEl.textContent = (typeof currentDrawLayer !== 'undefined' && currentDrawLayer===layerName) ? '◉' : '○';
      currentEl.onclick=(e)=>{
        e.preventDefault(); e.stopPropagation();
        if(typeof currentDrawLayer !== 'undefined') currentDrawLayer=layerName;
        const sel=document.getElementById('currentLayerSelect');
        if(sel) sel.value=layerName;
        command(`LAYER\n現在レイヤ：${layerName}`);
        setTimeout(rebuildLayerPanelRowsV72,0);
      };
    }else{
      currentEl.classList.add('layerCurrentMarkV72');
    }

    // 色チップ
    const colorHolder=document.createElement('span');
    colorHolder.className='layerColorInlineV72';
    colorHolder.appendChild(makeColorChipPickerV70(getLayerColorV70(layerName), (v)=>{
      setLayerColorV70(layerName, v==='BYLAYER'?7:v);
      command(`LAYER\nレイヤー「${layerName}」の色を変更：${colorLabelV691(v)}`);
      draw();
      setTimeout(rebuildLayerPanelRowsV72,0);
    ...
    nameSpan.className='layerNameV72';
    nameSpan.textContent=layerName;

    // 既存行を壊さない範囲で一度中身を整理
    row.innerHTML='';
    row.classList.add('layerRowV72','layerRowRebuiltV72');
    row.dataset.layerRowV72Done='1';

    if(eyeEl) row.appendChild(eyeEl);
    if(lockEl) row.appendChild(lockEl);
    row.appendChild(colorHolder);
...
(function(){
  const run=()=>setTimeout(rebuildLayerPanelRowsV72,0);
  setTimeout(rebuildLayerPanelRowsV72,300);
  document.addEventListener('click', e=>{ if(e.target && e.target.closest('.layerPanel')) run(); }, true);
  document.addEventListener('change', e=>{ if(e.target && e.target.closest('.layerPanel')) run(); }, true);
})();
...
// v80: レイヤー一覧を縦リストに作り直す
function rebuildLayerPanelRowsV73(){
  const panel=document.querySelector('.layerPanel');
  if(!panel || typeof allLayers !== 'function') return;

  // 入力欄・追加ボタン周辺を保持する
  const input = panel.querySelector('input');
  const addBtn = Array.from(panel.querySelectorAll('button')).find(b=>(b.textContent||'').includes('追加'));

  // 既存のレイヤー行/誤配置色チップを削除
  panel.querySelectorAll('.layerRowV72,.layerRowRebuiltV72,.layerRowV73,.layerListV73,.layerColorChipInlineV71,.layerColorHolderV70,.layerColorHolderV72').forEach(e=>e.remove());

  const list=document.createElement('div');
  list.className='layerListV73';

  const layers=allLayers();
  layers.forEach(layerName=>{
    const row=document.createElement('div');
    row.className='layerRowV73';
    ...
    eye.onclick=(e)=>{
      e.preventDefault(); e.stopPropagation();
      if(typeof layerVisible !== 'undefined' && layerVisible instanceof Map){
        layerVisible.set(layerName, !layerVisible.get(layerName));
      }
      command(`${layerName}：${(typeof layerVisible !== 'undefined' && layerVisible.get(layerName))?'表示':'非表示'}`);
      draw();
    };

    const lock=document.createElement('button');
    lock.type='button';
    const locked = (typeof layerLocked !== 'undefined' && layerLocked instanceof Map) ? !!layerLocked.get(layerName) : false;
    lock.textContent=locked?'🔒':'🔓';
    lock.title='ロック/アンロック';
    lock.onclick=(e)=>{
      e.preventDefault(); e.stopPropagation();
      if(typeof layerLocked !== 'undefined' && layerLocked instanceof Map){
        layerLocked.set(layerName, !layerLocked.get(layerName));
      }
      command(`${layerName}：${(typeof layerLocked !== 'undefined' && layerLocked.get(layerName))?'ロック':'アンロック'}`);
      setTimeout(rebuildLayerPanelRowsV73,0);
      draw();
    };

    const color=document.createElement('span');
    color.className='layerColorInlineV73';
    color.appendChild(makeColorChipPickerV70(getLayerColorV70(layerName), (v)=>{
      setLayerColorV70(layerName, v==='BYLAYER'?7:v);
      command(`LAYER\nレイヤー「${layerName}」の色を変更：${colorLabelV691(v)}`);
      draw();
      setTimeout(rebuildLayerPanelRowsV73,0);
    ...
    current.type='button';
    current.className='layerCurrentMarkV73';
    current.textContent=(typeof currentDrawLayer !== 'undefined' && currentDrawLayer===layerName)?'◉':'○';
    current.title='現在レイヤーにする';
    current.onclick=(e)=>{
      e.preventDefault(); e.stopPropagation();
      if(typeof currentDrawLayer !== 'undefined') currentDrawLayer=layerName;
      const sel=document.getElementById('currentLayerSelect');
    ...
    const name=document.createElement('span');
    name.className='layerNameV73';
    name.textContent=layerName;

    row.append(eye, lock, color, current, name);
    list.appendChild(row);
  });

  // 挿入位置：入力欄の前。なければ末尾。
  if(input && input.parentElement && input.parentElement.parentElement===panel){
    panel.insertBefore(list, input.parentElement);
  } else if(input && input.parentElement===panel){
    panel.insertBefore(list, input);
  } else {
...
(function(){
  const run=()=>setTimeout(rebuildLayerPanelRowsV73,0);
  setTimeout(rebuildLayerPanelRowsV73,300);
  document.addEventListener('click', e=>{ if(e.target && e.target.closest('.layerPanel')) run(); }, true);
  document.addEventListener('change', e=>{ if(e.target && e.target.closest('.layerPanel')) run(); }, true);
})();
...
// v80: レイヤー色パレットが一瞬で閉じる問題を修正
function openLayerColorPaletteV74(layerName, anchor){
  closeLayerColorPaletteV74();
  const pal=document.createElement('div');
  pal.className='floatingColorPaletteV74';
  pal.dataset.layer=layerName;

  ['1','2','3','4','5','6','7','8','9'].forEach(k=>{
    const btn=document.createElement('button');
    btn.type='button';
    btn.className='colorChipButtonV70';
    btn.title=colorLabelV691(k);
    btn.appendChild(makeColorChipV691(k));
    btn.onclick=(e)=>{
      e.preventDefault();
      e.stopPropagation();
      setLayerColorV70(layerName, Number(k));
      command(`LAYER\nレイヤー「${layerName}」の色を変更：${colorLabelV691(k)}`);
      closeLayerColorPaletteV74();
      draw();
      setTimeout(rebuildLayerPanelRowsV73,0);
      setTimeout(stabilizeLayerColorChipsV74,30);
    };
    pal.appendChild(btn);
  });
  ...
  pal.style.left=Math.max(4, Math.min(window.innerWidth-180, r.left))+'px';
  pal.style.top=Math.max(4, Math.min(window.innerHeight-90, r.bottom+4))+'px';

  setTimeout(()=>document.addEventListener('pointerdown', closePaletteOnOutsideV74, true),0);
}
function closePaletteOnOutsideV74(e){
  const pal=document.querySelector('.floatingColorPaletteV74');
  if(!pal) return;
  if(e && (pal.contains(e.target) || (e.target && e.target.closest('.layerColorInlineV73')))) return;
  closeLayerColorPaletteV74();
}
function closeLayerColorPaletteV74(){
  document.removeEventListener('pointerdown', closePaletteOnOutsideV74, true);
  document.querySelectorAll('.floatingColorPaletteV74').forEach(p=>p.remove());
}
function stabilizeLayerColorChipsV74(){
  const panel=document.querySelector('.layerPanel');
  if(!panel) return;
  panel.querySelectorAll('.layerRowV73').forEach(row=>{
    const layerName=(row.querySelector('.layerNameV73')?.textContent||'').trim();
    const slot=row.querySelector('.layerColorInlineV73');
    if(!layerName || !slot) return;
    slot.innerHTML='';
    const btn=document.createElement('button');
    btn.type='button';
    btn.className='colorChipButtonV70 current';
    btn.title='レイヤー色';
    btn.appendChild(makeColorChipV691(getLayerColorV70(layerName)));
    btn.onclick=(e)=>{
      e.preventDefault();
      e.stopPropagation();
      openLayerColorPaletteV74(layerName, btn);
    };
    slot.appendChild(btn);
  });
}
(function(){
  const run=()=>setTimeout(stabilizeLayerColorChipsV74,20);
  setTimeout(run,350);
  document.addEventListener('click', e=>{ if(e.target && e.target.closest('.layerPanel')) run(); }, true);
  document.addEventListener('change', e=>{ if(e.target && e.target.closest('.layerPanel')) run(); }, true);
})();
...
// v80: レイヤー名が長い場合、レイヤー一覧内で横スクロールできるようにする
function enableLayerPanelScrollV75(){
  const panel=document.querySelector('.layerPanel');
  if(!panel) return;
  const list=panel.querySelector('.layerListV73');
  if(list){
    list.style.overflowX='auto';
    list.style.overflowY='auto';
    list.style.webkitOverflowScrolling='touch';
  }
  panel.querySelectorAll('.layerNameV73').forEach(el=>{
...
(function(){
  const run=()=>setTimeout(enableLayerPanelScrollV75,0);
  setTimeout(run,300);
  document.addEventListener('click', e=>{ if(e.target && e.target.closest('.layerPanel')) run(); }, true);
  document.addEventListener('change', e=>{ if(e.target && e.target.closest('.layerPanel')) run(); }, true);
})();
...
// v80: ツールバーの現在レイヤーとレイヤーパネルの現在レイヤーを双方向同期
function syncCurrentLayerUIV76(layerName){
  try{
    if(layerName && typeof currentDrawLayer !== 'undefined') currentDrawLayer = layerName;
    const current = (typeof currentDrawLayer !== 'undefined' && currentDrawLayer) ? currentDrawLayer : '0';
    ...
    document.querySelectorAll('.layerRowV73').forEach(row=>{
      const name=(row.querySelector('.layerNameV73')?.textContent||'').trim();
      const mark=row.querySelector('.layerCurrentMarkV73');
      if(mark) mark.textContent = (name===current) ? '◉' : '○';
      row.classList.toggle('activeLayerV76', name===current);
    });
  }catch(e){
    console.warn('syncCurrentLayerUIV76 failed', e);
  }
}

function attachCurrentLayerToolbarSyncV76(){
  const sel=document.getElementById('currentLayerSelect');
  if(!sel || sel.dataset.v80LayerSync==='1') return;
  sel.dataset.v80LayerSync='1';
  sel.addEventListener('change', ()=>{
    syncCurrentLayerUIV76(sel.value);
    command(`LAYER\n現在レイヤ：${sel.value}`);
    setTimeout(()=>{
      if(typeof rebuildLayerPanelRowsV73==='function') rebuildLayerPanelRowsV73();
      syncCurrentLayerUIV76(sel.value);
    },0);
  });
}
...
(function(){
  const run=()=>setTimeout(()=>{
    attachCurrentLayerToolbarSyncV76();
    syncCurrentLayerUIV76();
  },0);

  setTimeout(run,300);
  document.addEventListener('change', e=>{
    if(e.target && (e.target.id==='currentLayerSelect' || e.target.closest('.layerPanel'))) run();
  }, true);
  document.addEventListener('click', e=>{
    if(e.target && e.target.closest('.layerPanel')) run();
  }, true);
})();



// v80: ツールバー現在色のBYLAYERは現在レイヤー色として表示
function currentDrawEffectiveColorV77(){
  const cur = window.currentDrawColorV691 || localStorage.getItem('surveycad.currentColorV691') || 'BYLAYER';
  if(String(cur).toUpperCase() !== 'BYLAYER') return cur;
  const layer = (typeof currentDrawLayer !== 'undefined' && currentDrawLayer) ? currentDrawLayer : '0';
  return getLayerColorV70(layer);
}
function updateCurrentColorChipV77(){
  try{
    const wrap=document.getElementById('currentColorControlV70') || document.getElementById('currentColorControlV691');
    if(!wrap) return;
    const btn=wrap.querySelector('.colorChipButtonV70.current, .colorChipButtonV70');
    if(!btn) return;
    const cur = window.currentDrawColorV691 || localStorage.getItem('surveycad.currentColorV691') || 'BYLAYER';
    const effective = currentDrawEffectiveColorV77();
    btn.innerHTML='';
    btn.appendChild(makeColorChipV691(effective));
    btn.title = String(cur).toUpperCase()==='BYLAYER'
      ? `BYLAYER：現在レイヤー色 ${colorLabelV691(effective)}`
      : colorLabelV691(cur);
    btn.classList.toggle('bylayerEffectiveV77', String(cur).toUpperCase()==='BYLAYER');
  }catch(e){}
}
(function(){
  setTimeout(updateCurrentColorChipV77,300);
  document.addEventListener('change', e=>{
    if(e.target && (e.target.id==='currentLayerSelect' || e.target.closest('.layerPanel') || e.target.closest('#currentColorControlV70'))) setTimeout(updateCurrentColorChipV77,0);
  }, true);
  document.addEventListener('click', e=>{
    if(e.target && (e.target.closest('.layerPanel') || e.target.closest('#currentColorControlV70'))) setTimeout(updateCurrentColorChipV77,40);
  }, true);
})();



// v80: ツールバー現在色を「色チップのみ」に統一
// - 「現在色」文字と色名selectを非表示
// - 色チップをタップして BYLAYER / 固定色 を選択
// - BYLAYERは現在レイヤー色で表示し、点線枠
function closeToolbarColorPaletteV78(){
  document.querySelectorAll('.toolbarColorPaletteV78').forEach(p=>p.remove());
  document.removeEventListener('pointerdown', closeToolbarColorPaletteOutsideV78, true);
}
function closeToolbarColorPaletteOutsideV78(e){
  const pal=document.querySelector('.toolbarColorPaletteV78');
  if(!pal) return;
  if(e && (pal.contains(e.target) || (e.target && e.target.closest('#toolbarCurrentColorChipV78')))) return;
  closeToolbarColorPaletteV78();
}
function openToolbarColorPaletteV78(anchor){
  closeToolbarColorPaletteV78();
  const pal=document.createElement('div');
  pal.className='toolbarColorPaletteV78';

  const keys=['BYLAYER','1','2','3','4','5','6','7','8','9'];
  keys.forEach(k=>{
    const btn=document.createElement('button');
    btn.type='button';
    btn.className='colorChipButtonV70';
    btn.title=(k==='BYLAYER')?'BYLAYER':colorLabelV691(k);
    const chipVal = k==='BYLAYER' ? currentDrawEffectiveColorV77() : k;
    btn.appendChild(makeColorChipV691(chipVal));
    if(k==='BYLAYER') btn.classList.add('bylayerEffectiveV77');
    btn.onclick=(e)=>{
      e.preventDefault();
      e.stopPropagation();
      window.currentDrawColorV691 = k;
      localStorage.setItem('surveycad.currentColorV691', k);
      closeToolbarColorPaletteV78();
      updateToolbarCurrentColorChipV78();
      command(`現在色：${k==='BYLAYER'?'BYLAYER':colorLabelV691(k)}`);
    };
    pal.appendChild(btn);
  });
  ...
  document.body.appendChild(pal);
  const r=anchor.getBoundingClientRect();
  pal.style.left=Math.max(4, Math.min(window.innerWidth-190, r.left))+'px';
  pal.style.top=Math.max(4, Math.min(window.innerHeight-96, r.bottom+4))+'px';
  setTimeout(()=>document.addEventListener('pointerdown', closeToolbarColorPaletteOutsideV78, true),0);
}
function updateToolbarCurrentColorChipV78(){
  const holder=document.getElementById('toolbarCurrentColorChipV78');
  if(!holder) return;
  const cur = window.currentDrawColorV691 || localStorage.getItem('surveycad.currentColorV691') || 'BYLAYER';
  const effective = (String(cur).toUpperCase()==='BYLAYER') ? currentDrawEffectiveColorV77() : cur;
  holder.innerHTML='';
  const btn=document.createElement('button');
  btn.type='button';
  btn.className='colorChipButtonV70 current';
  if(String(cur).toUpperCase()==='BYLAYER') btn.classList.add('bylayerEffectiveV77');
  btn.title = String(cur).toUpperCase()==='BYLAYER'
    ? `BYLAYER：現在レイヤー色 ${colorLabelV691(effective)}`
    : colorLabelV691(cur);
  btn.appendChild(makeColorChipV691(effective));
  btn.onclick=(e)=>{
    e.preventDefault();
    e.stopPropagation();
    openToolbarColorPaletteV78(btn);
  };
  holder.appendChild(btn);
}
function rebuildToolbarColorControlV78(){
  // 古い「現在色」UIを消す
  const old1=document.getElementById('currentColorControlV70');
  if(old1) old1.remove();
  const old2=document.getElementById('currentColorControlV691');
  if(old2) old2.remove();

  if(document.getElementById('toolbarCurrentColorChipV78')) {
    updateToolbarCurrentColorChipV78();
    return;
  }
  ...
  const label=document.createElement('label');
  label.id='toolbarCurrentColorChipV78';
  label.className='toolbarCurrentColorChipV78';
  layerSel.parentElement.insertAdjacentElement('afterend', label);
  updateToolbarCurrentColorChipV78();
}
...
(function(){
  ...
  document.addEventListener('click', e=>{
    if(e.target && (e.target.closest('.layerPanel') || e.target.closest('#toolbarCurrentColorChipV78'))) setTimeout(updateToolbarCurrentColorChipV78,40);
  }, true);
})();



// v80: ツールバー色UIを [レイヤー▼][■] のみに整理
function removeOldToolbarColorTextV79(){
  ['currentColorControlV70','currentColorControlV691','toolbarCurrentColorChipV78'].forEach(id=>{
    const el=document.getElementById(id);
    if(el) el.remove();
  });
  Array.from(document.querySelectorAll('label,span,div')).forEach(el=>{
    const txt=(el.textContent||'').trim();
    if((txt==='色' || txt==='現在色' || txt.startsWith('現在色')) &&
       !el.closest('.layerPanel') && !el.closest('.propPanel') &&
       !el.contains(document.getElementById('currentLayerSelect'))){
      el.remove();
    }
  });
  Array.from(document.querySelectorAll('select')).forEach(sel=>{
    if(sel.id==='currentLayerSelect') return;
    if(sel.closest('.layerPanel') || sel.closest('.propPanel')) return;
    const opts=Array.from(sel.options||[]).map(o=>o.textContent.trim()).join('|');
    if(/BYLAYER|赤|黄|緑|シアン|青|マゼンタ|白|グレー/.test(opts)){
      sel.remove();
    }
  });
}
function closeToolbarColorPaletteV79(){
  document.querySelectorAll('.toolbarColorPaletteV79').forEach(p=>p.remove());
  document.removeEventListener('pointerdown', closeToolbarColorPaletteOutsideV79, true);
}
function closeToolbarColorPaletteOutsideV79(e){
  const pal=document.querySelector('.toolbarColorPaletteV79');
  if(!pal) return;
  if(e && (pal.contains(e.target) || (e.target && e.target.closest('#toolbarCurrentColorChipV79')))) return;
  closeToolbarColorPaletteV79();
}
function openToolbarColorPaletteV79(anchor){
  closeToolbarColorPaletteV79();
  const pal=document.createElement('div');
  pal.className='toolbarColorPaletteV79';
  ['BYLAYER','1','2','3','4','5','6','7','8','9'].forEach(k=>{
    const btn=document.createElement('button');
    btn.type='button';
    btn.className='colorChipButtonV70';
    if(k==='BYLAYER') btn.classList.add('bylayerEffectiveV77');
    const cur=window.currentDrawColorV691 || localStorage.getItem('surveycad.currentColorV691') || 'BYLAYER';
    if(String(cur).toUpperCase()===String(k).toUpperCase()) btn.classList.add('selected');
    btn.title=k==='BYLAYER'?'BYLAYER':colorLabelV691(k);
    btn.appendChild(makeColorChipV691(k==='BYLAYER'?currentDrawEffectiveColorV77():k));
    btn.onclick=(e)=>{
      e.preventDefault(); e.stopPropagation();
      window.currentDrawColorV691=k;
      localStorage.setItem('surveycad.currentColorV691',k);
      closeToolbarColorPaletteV79();
      updateToolbarCurrentColorChipV79();
      command(`現在色：${k==='BYLAYER'?'BYLAYER':colorLabelV691(k)}`);
    };
    pal.appendChild(btn);
  });
  ...
  pal.style.left=Math.max(4,Math.min(window.innerWidth-190,r.left))+'px';
  pal.style.top=Math.max(4,Math.min(window.innerHeight-96,r.bottom+4))+'px';
  setTimeout(()=>document.addEventListener('pointerdown',closeToolbarColorPaletteOutsideV79,true),0);
}
function updateToolbarCurrentColorChipV79(){
  let holder=document.getElementById('toolbarCurrentColorChipV79');
  if(!holder){
    const layerSel=document.getElementById('currentLayerSelect');
    if(!layerSel || !layerSel.parentElement) return;
    holder=document.createElement('label');
    holder.id='toolbarCurrentColorChipV79';
    holder.className='toolbarCurrentColorChipV79';
    layerSel.parentElement.insertAdjacentElement('afterend',holder);
  }
  holder.innerHTML='';
  const cur=window.currentDrawColorV691 || localStorage.getItem('surveycad.currentColorV691') || 'BYLAYER';
  const effective=String(cur).toUpperCase()==='BYLAYER'?currentDrawEffectiveColorV77():cur;
  const btn=document.createElement('button');
  btn.type='button';
  btn.className='colorChipButtonV70 current toolbarColorButtonV79';
  if(String(cur).toUpperCase()==='BYLAYER') btn.classList.add('bylayerEffectiveV77');
  btn.title=String(cur).toUpperCase()==='BYLAYER'?`BYLAYER：現在レイヤー色 ${colorLabelV691(effective)}`:colorLabelV691(cur);
  btn.appendChild(makeColorChipV691(effective));
  btn.onclick=(e)=>{e.preventDefault();e.stopPropagation();openToolbarColorPaletteV79(btn);};
  holder.appendChild(btn);
}
(function(){
  const run=()=>setTimeout(()=>{removeOldToolbarColorTextV79();updateToolbarCurrentColorChipV79();},0);
  setTimeout(run,400);
  document.addEventListener('change',e=>{if(e.target&&(e.target.id==='currentLayerSelect'||e.target.closest('.layerPanel')))run();},true);
  document.addEventListener('click',e=>{if(e.target&&(e.target.closest('.layerPanel')||e.target.closest('#toolbarCurrentColorChipV79')))setTimeout(updateToolbarCurrentColorChipV79,40);},true);
})();



// v80: 新規作図・生成オブジェクトへ現在色を確実に適用
function getCurrentDrawColorForEntityV80(){
  const cur = window.currentDrawColorV691 || localStorage.getItem('surveycad.currentColorV691') || 'BYLAYER';
  if(String(cur).toUpperCase()==='BYLAYER') return 'BYLAYER';
  const n=Number(cur);
  return Number.isFinite(n) ? n : 'BYLAYER';
}
function applyCurrentColorToEntityV80(e){
  if(!e) return e;
  if(e.color === undefined || e.color === null || e.color === ''){
    e.color = getCurrentDrawColorForEntityV80();
  }
  return e;
}
function normalizeCreatedEntityColorsV80(){
  try{
    if(!Array.isArray(entities)) return;
    entities.forEach(e=>{
      if(e && e.created && (e.color === undefined || e.color === null || e.color === '')){
        e.color = getCurrentDrawColorForEntityV80();
      }
    });
  }catch(e){}
}
function resolvedEntityColorCssV80(e, fallback='#ffffff'){
  try{
    const c = e && e.color !== undefined ? e.color : 'BYLAYER';
    if(String(c).toUpperCase()==='BYLAYER'){
      const layer = e && e.layer ? e.layer : '0';
      return colorCssV691(getLayerColorV70(layer), fallback);
    }
    return colorCssV691(c, fallback);
  }catch(err){ return fallback; }
}
(function(){
  const oldDraw = window.draw;
  if(typeof oldDraw === 'function' && !oldDraw.__v80Wrapped){
    window.draw=function(){
      normalizeCreatedEntityColorsV80();
      return oldDraw.apply(this, arguments);
    };
    window.draw.__v80Wrapped=true;
  }
})();
